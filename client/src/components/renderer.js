...
/**
 * Renders a vertical Deep Search roadmap/stepper with connected numbered circles.
 * Format requested by user:
 *   (circle) 1 Plan title
 *      (dot dot line)
 *   (circle) 2 Plan title ...
 */
export function renderDeepSearchStepper(rawText) {
    ...
    for (const line of lines) {
        ...
        // Status tags: [done], [active], [pending], [running]
        if (/^\[(done|completed|ok|x)\]/i.test(text)) {
            status = "done";
            text = text.replace(/^\[(done|completed|ok|x)\]\s*/i, "");
        } else if (/^\[(active|in-progress|running|current|\*)\]/i.test(text)) {
            status = "active";
            text = text.replace(/^\[(active|in-progress|running|current|\*)\]\s*/i, "");
        } else if (/^\[(pending|todo)\]/i.test(text)) {
            status = "pending";
            text = text.replace(/^\[(pending|todo)\]\s*/i, "");
        }

        // Check for numbered prefix: "1.", "1)", "(1)", "Step 1:"
        let num = autoIndex;
        const numMatch = text.match(/^(?:step\s+)?(?:\(?(\d+)\)?[\.\:\)]|\d+\.)\s*(.*)/i);
        if (numMatch) {
            num = parseInt(numMatch[1], 10) || autoIndex;
        ...
        // Split into title and description if separator exists
        let title = text;
        let desc = "";
        const sepMatch = text.match(/^([^:\-–—\n]+?)\s*[:\-–—]\s+(.+)$/);
        if (sepMatch && sepMatch[1].trim().length <= 60) {
            title = sepMatch[1].trim();
            desc = sepMatch[2].trim();
        }

        steps.push({ num, title, desc, status });
        autoIndex = num + 1;
    }
    ...
    const itemsHtml = steps.map((s, idx) => {
        ...
        return `
        <div class="stepper-item ${s.status}">
            <div class="stepper-rail">
                <div class="stepper-circle" title="Step ${s.num}">
                    ${s.status === "done" ? '<i data-lucide="check" class="stepper-check-icon"></i>' : s.num}
                </div>
                ${!isLast ? '<div class="stepper-line" aria-hidden="true"></div>' : ''}
            </div>
            <div class="stepper-body">
                <div class="stepper-step-title">${escapeHTML(s.title)}</div>
                ${s.desc ? `<div class="stepper-step-desc">${escapeHTML(s.desc)}</div>` : ''}
            </div>
        </div>
        `;
    }).join("");
    ...
    return `
    <div class="deepsearch-plan-card">
        <div class="deepsearch-plan-header">
            <i data-lucide="compass" class="deepsearch-plan-icon"></i>
            <span class="deepsearch-plan-title">Deep Research Plan</span>
            <span class="deepsearch-plan-badge">${steps.length} Steps</span>
        </div>
        <div class="deepsearch-stepper">
            ${itemsHtml}
        </div>
    </div>
    `;
...
export function parseMarkdown(text) {
    ...
    if (content) {
        ...
        const planNormalizedText = processedText.replace(/<\s*(?:plan|stepper)\s*>([\s\S]*?)<\s*\/\s*(?:plan|stepper)\s*>/gi, (_, planCode) => {
            return `\n\n\`\`\`plan\n${planCode.trim()}\n\`\`\`\n\n`;
        });
        if (typeof marked !== "undefined" && typeof marked.parse === "function") {
            try {
                parsedContent = marked.parse(planNormalizedText);
            } catch (e) {
                parsedContent = escapeHTML(planNormalizedText);
            }
        } else {
            parsedContent = escapeHTML(planNormalizedText);
        }
